...
<head>
  ...
  <title>Würzburg All Trees Gardener Map</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .treeCard.sensor { border-color:#7ee6a2; }
    .treeCard.inventory { opacity:.9; }
    .filterRow { display:flex; gap:8px; padding:0 16px 12px; }
    .filterRow button { font-size:12px; padding:8px 10px; background:#102c1a; border:1px solid #315a3e; }
    .filterRow button.active { background:linear-gradient(135deg,#15803d,#2563eb); }
...
<body>
  <header>
    <div><div class="eyebrow">Würzburg OpenData · all tree cadastre + live sensors</div><h1>All Trees Gardener Map</h1><p class="subtitle">Maps the full Würzburg tree cadastre and highlights the live climate-tree moisture sensors. Hover a marker or card to fade details into view.</p></div>
    <div class="statusBox"><b id="treeCount">—</b><span id="generatedAt">Loading all OpenData trees…</span></div>
  </header>
  <section class="layout">
    <div class="panel"><div id="map"></div></div>
    <aside class="side panel"><div class="cards" id="summary"></div><div class="filterRow"><button id="showPriority" class="active">Priority cards</button><button id="showInventory">Inventory sample</button></div><div class="cards" id="cards"></div><div class="detail" id="detail"><h2>Hover a tree</h2><p>Cadastre trees show species/size/location. Climate sensor trees also show moisture, battery, temperature, action hint, and mini history.</p></div></aside>
  </section>
  ...
  <footer>Source: <a href="https://opendata.wuerzburg.de/" style="color:#9fffc1">opendata.wuerzburg.de</a> · Datasets: <code>baumkataster_stadt_wuerzburg</code> + <code>sls-klimabaeume</code></footer>
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js" crossorigin=""></script>
<script>
const treeCount = document.getElementById('treeCount'), generatedAt = document.getElementById('generatedAt'), cards = document.getElementById('cards'), summary = document.getElementById('summary'), detail = document.getElementById('detail');
...
let snapshot = null, map = null, markers = new Map(), cardMode = 'priority';
const esc = s => String(s ?? '').replace(/[&<>'"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
function color(level){ return {critical:'#ff6b6b',watch:'#f5c84b',ok:'#44d27d',good:'#44d27d',inventory:'#86c5ff',unknown:'#9bb9a5'}[level] || '#9bb9a5'; }
function fmt(value, fallback='—'){ return value === null || value === undefined || value === '' ? fallback : esc(value); }
function spark(history){
  const vals = history.map(h => Number(h.vwc)).filter(Number.isFinite); if(vals.length < 2) return '<div class="treeMeta">No sensor history</div>';
...
function renderDetail(t){
  const p=t.priority || {}, sensor=!!t.hasSensor;
  detail.innerHTML = `<h2>${esc(t.displayName || t.origin || t.species)}</h2><p><span class="pill ${esc(p.level)}">${esc(p.label)} · ${sensor ? 'sensor' : 'cadastre'}</span></p>${spark(t.history || [])}<dl><dt>Species</dt><dd>${fmt(t.species)}${t.species_latin ? ` · <em>${esc(t.species_latin)}</em>` : ''}</dd><dt>Tree ID</dt><dd>${fmt(t.tree_id || t.serial)}</dd><dt>Moisture</dt><dd>${sensor ? `${fmt(t.vwc)}% VWC · trend ${fmt(t.vwcTrend)}` : 'No live moisture sensor on this cadastre tree'}</dd><dt>Temperature</dt><dd>${sensor ? `${fmt(t.temperature)} °C` : '—'}</dd><dt>Battery</dt><dd>${sensor ? `${fmt(t.battery)}%` : '—'}</dd><dt>Size</dt><dd>height ${fmt(t.height_m)} m · crown ${fmt(t.crown_width_m)} m · trunk ${fmt(t.trunk_circumference_cm)} cm</dd><dt>Latest</dt><dd>${fmt(t.timestamp)}</dd><dt>Action</dt><dd>${esc(p.action || '')}</dd></dl>`;
  document.querySelectorAll('.treeCard').forEach(el => el.classList.toggle('active', el.dataset.id === t.id));
}
function card(t){ const p=t.priority||{}, sensor=!!t.hasSensor; return `<article class="treeCard ${sensor?'sensor':'inventory'}" data-id="${esc(t.id)}"><div class="treeTop"><div><div class="treeTitle">${esc(t.displayName || t.origin)}</div><div class="treeMeta">${esc(t.species || '')} · ${sensor ? `${t.historyCount} sensor records` : 'cadastre tree'}</div></div><div class="metric">${sensor ? esc(t.vwc) : esc(t.height_m || '—')}<small>${sensor ? '% VWC' : 'm height'}</small></div></div><span class="pill ${esc(p.level)}">${esc(p.label)}</span>${spark(t.history || [])}</article>`; }
function renderCards(){
  if(!snapshot) return;
  document.getElementById('showPriority').classList.toggle('active', cardMode==='priority');
  document.getElementById('showInventory').classList.toggle('active', cardMode==='inventory');
  const list = cardMode === 'priority' ? snapshot.trees.filter(t => t.hasSensor).concat(snapshot.trees.filter(t => !t.hasSensor).slice(0, 20)) : snapshot.trees.filter(t => !t.hasSensor).slice(0, 80);
  cards.innerHTML = list.map(card).join('') + `<div class="treeMeta">Showing ${list.length} cards. The map renders all ${snapshot.treeCount} trees.</div>`;
  cards.querySelectorAll('.treeCard').forEach(el => el.addEventListener('mouseover',()=>renderDetail(snapshot.trees.find(t=>t.id===el.dataset.id))));
}
function initMap(trees){
  if(!map){ map = L.map('map', {preferCanvas:true}); L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {maxZoom:19, attribution:'© OpenStreetMap'}).addTo(map); }
  markers.forEach(m => m.remove()); markers.clear(); const bounds=[];
  const renderer = L.canvas({padding:0.5});
  trees.forEach(t => { const p=t.priority||{}, c=color(p.level), sensor=!!t.hasSensor; const m=L.circleMarker([t.lat,t.lon], {renderer, radius:sensor?11:3, color:c, fillColor:c, fillOpacity:sensor ? .88 : .45, weight:sensor?3:1}).addTo(map); m.bindTooltip(`${t.displayName || t.origin}: ${sensor ? `${t.vwc}% VWC · ` : ''}${p.label}`); m.on('mouseover',()=>renderDetail(t)); m.on('click',()=>renderDetail(t)); markers.set(t.id,m); bounds.push([t.lat,t.lon]); });
  if(bounds.length) map.fitBounds(bounds, {padding:[40,40], maxZoom:14});
}
async function load(){ const res=await fetch('/api/trees'); const data=await res.json(); if(!res.ok) throw new Error(data.error||'snapshot failed'); snapshot=data; treeCount.textContent=`${data.treeCount.toLocaleString()} trees`; generatedAt.textContent=`Generated ${data.generatedAt} · ${data.inventoryTreeCount.toLocaleString()} cadastre + ${data.sensorTreeCount} sensor trees`; summary.innerHTML=`<article class="treeCard"><b>${data.inventoryTreeCount.toLocaleString()} cadastre trees</b><div class="treeMeta">Full city tree inventory from Baumkataster</div></article><article class="treeCard sensor"><b>${data.sensorTreeCount} live sensor trees</b><div class="treeMeta">${data.dryOrWatchCount} dry/watch · avg ${fmt(data.averageVwc)}% VWC</div></article>`; renderCards(); initMap(data.trees); if(data.trees[0]) renderDetail(data.trees[0]); }
...
async function ask(q){ msg('user', q); const pending=msg('', 'Thinking against the current tree snapshot…'); const res=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question:q})}); const data=await res.json(); if(!res.ok) throw new Error(data.error||'chat failed'); pending.textContent=`${data.answer}\n\n— ${data.model}, context ${data.contextGeneratedAt}`; }
document.getElementById('showPriority').addEventListener('click', () => { cardMode='priority'; renderCards(); });
document.getElementById('showInventory').addEventListener('click', () => { cardMode='inventory'; renderCards(); });
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if not question:
                ...
                return
            snapshot = request.get("snapshot") if isinstance(request.get("snapshot"), dict) else build_snapshot()
